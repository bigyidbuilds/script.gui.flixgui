...
from FlixGui.modules._common import DateTimeNow,Log
from FlixGui.modules import media_player 
...
class WindowItemMovie(xbmcgui.WindowXML):
	...
	def __init__(self,dbconn,listitem,*args,**kwargs):
		...
		self.ratingfolder_path = os.path.join(self.scriptPath,'resources','media','age_rating_icons')
		...
		try:
			self.ratingicon = os.path.join(self.ratingfolder_path,xbmcaddon.Addon(self.addon_id).getSetting("general.region"),'{}.png'.format(self.listitem.getProperty('age_rating')))
		except:
			self.ratingicon = os.path.join(self.ratingfolder_path,xbmcaddon.Addon(self.addon_id).getSetting("general.region"),'AV.png')
		self.visibleButtons = []
	...
	def onClick(self,controlId):
		...
		if controlId in [self.RATINGBUTTON_1,self.RATINGBUTTON_2]:
			...
			d=window_credits.WindowCredits(self.dbconn,self.movieItem.credits_cast,self.movieItem.credits_crew,self.movieItem.videos,self.listitem.getUniqueID('tmdb'))
			d.doModal()
			del d
