...
class WindowMore(xbmcgui.WindowXML):
	...
	def AgeRateIcon(self,age_rating):
		try:
			ratingicon = os.path.join(self.ratingfolder_path,xbmcaddon.Addon(self.addon_id).getSetting("general.region"),'{}.png'.format(age_rating))
		except:
			ratingicon = os.path.join(self.ratingfolder_path,xbmcaddon.Addon(self.addon_id).getSetting("general.region"),'AV.png')
		return ratingicon