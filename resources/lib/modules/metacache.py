		...
		if d:
			...
			for release_dates_result in release_dates_results:
				if ("iso_3166_1",xbmcaddon.Addon(self.addon_id).getSetting("general.region")) in release_dates_result.items():
					...
					for r in rd:
						age_rating = r.get('certification')
						if not release_date:
							try:
			...
			with self.dbconn.conn: 
				self.dbconn.conn.execute("INSERT OR IGNORE INTO master.tmdb_movie_details(tmdb_id, backdrop_path, overview, poster_path , release_date , title , vote_count , genres ,vote_average ,media_type,age_rating,runtime,credits_cast,credits_crew,videos) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(tmdbid,backdrop_path,d.get('overview'),poster_path,release_date,d.get('title'),d.get('vote_count'),str(d.get('genres')),d.get('vote_average'),'movie',age_rating,d.get("runtime"),str(credits.get('cast')),str(credits.get('crew')),str(d.get('videos').get('results'))))
				self.dbconn.conn.commit()

	def TvMeta(self,tmdb_id):
		d = self.tmdbapi.tv(tmdbid,'Details',append_to_response='videos,content_ratings,credits')
		if d:
			content_ratings = list(filter(lambda r: r["iso_3166_1"] == self.setting("general.region"), d.get('content_ratings').get('results')))
			credits = d.get('credits')
			...
			poster_path = self.ImagePath(d.get('poster_path'))
			with self.dbconn.conn:
				self.dbconn.conn.execute("INSERT OR IGNORE INTO master.tmdb_tv_details(tmdb_id, backdrop_path, overview, poster_path, first_air_date, title, vote_count, genres,vote_average ,media_type,total_seasons,videos,age_rating,credits_cast,credits_crew) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(tmdbid,backdrop_path,d.get('overview'),poster_path,first_air_date,d.get('name'),d.get('vote_count'),str(d.get('genres')),d.get('vote_average'),'tvshow',d.get('number_of_seasons'),str(d.get('videos').get('results')),content_rating,str(credits.get('cast')),str(credits.get('crew'))))
				self.dbconn.conn.commit()
